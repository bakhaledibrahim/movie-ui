    ...
    return (
        <div className="relative h-[56.25vw] min-h-[400px] max-h-[800px]">
            <img src={`https://image.tmdb.org/t/p/original${movie.backdrop_path}`}
                 className="w-full h-full object-cover" alt={movie.title || movie.name}/>
            <div className="absolute inset-0 bg-gradient-to-r from-black to-black opacity-40"></div>
            <div className="absolute bottom-0 left-0 right-0 h-3/4 bg-gradient-to-t from-brand-bg to-transparent"></div>
            <div className="absolute bottom-[20%] left-4 md:left-12 max-w-xl z-10">
                ...
                <div className="flex space-x-4 mt-4">
                    <Link to={getPlayerUrl()}
                          className="flex items-center bg-white text-black font-bold py-2 px-6 rounded hover:bg-gray-200 transition-transform hover:scale-105 active:scale-95">
                        <FaPlay className="mr-2"/> Play
                    </Link>
                    <button onClick={onMoreInfo}
                            className="flex items-center bg-gray-500 bg-opacity-70 text-white font-bold py-2 px-6 rounded hover:bg-opacity-50 transition-transform hover:scale-105 active:scale-95">
